"""
Backend API tests for SY Homes Entities module (Prompt 1.1)
...
def tenant_id(api_client):
    ...
    response = api_client.get(f"{BASE_URL}/api/meta/tenant")
    assert response.status_code == 200
    return response.json()["id"]


@pytest.fixture(scope="module")
def seeded_entities(api_client):
    """Get the 3 seeded entities"""
    response = api_client.get(f"{BASE_URL}/api/entities", params={"page_size": 100})
    assert response.status_code == 200
...
class TestHealthEndpoint:
    ...
    def test_health_returns_200(self, api_client):
        ...
        assert data["module"] == "entities"
        assert data["phase"] == "1.1"


class TestMetaEndpoints:
    """Meta endpoints: tenant, enums, insurance-alerts"""
    
    def test_tenant_returns_sy_homes(self, api_client):
        response = api_client.get(f"{BASE_URL}/api/meta/tenant")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_enums_returns_all_5_enums(self, api_client):
        response = api_client.get(f"{BASE_URL}/api/meta/enums")
        assert response.status_code == 200
        data = response.json()
...
class TestEntitiesListEndpoint:
    ...
    def test_list_returns_3_seeded_entities(self, api_client):
        response = api_client.get(f"{BASE_URL}/api/entities")
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 3
    ...
    def test_filter_by_entity_type_parent(self, api_client):
        response = api_client.get(f"{BASE_URL}/api/entities", params={"entity_type": "Parent"})
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 1
    ...
    def test_filter_by_entity_type_spv(self, api_client):
        response = api_client.get(f"{BASE_URL}/api/entities", params={"entity_type": "SPV"})
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 1
    ...
    def test_search_by_name(self, api_client):
        response = api_client.get(f"{BASE_URL}/api/entities", params={"q": "Shrewsbury"})
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 1
        assert "Shrewsbury" in data["items"][0]["name"]
    
    def test_sort_by_name_desc(self, api_client):
        response = api_client.get(f"{BASE_URL}/api/entities", params={"sort": "name", "dir": "desc"})
        assert response.status_code == 200
        data = response.json()
        names = [e["name"] for e in data["items"]]
        assert names == sorted(names, reverse=True)
    
    def test_sort_by_name_asc(self, api_client):
        response = api_client.get(f"{BASE_URL}/api/entities", params={"sort": "name", "dir": "asc"})
        assert response.status_code == 200
        data = response.json()
        names = [e["name"] for e in data["items"]]
        assert names == sorted(names)
    
    def test_invalid_entity_type_filter_returns_400(self, api_client):
        response = api_client.get(f"{BASE_URL}/api/entities", params={"entity_type": "InvalidType"})
...
class TestEntityDetailEndpoint:
    ...
    def test_parent_entity_has_children(self, api_client, parent_entity_id):
        response = api_client.get(f"{BASE_URL}/api/entities/{parent_entity_id}")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_child_entity_has_parent(self, api_client, seeded_entities, parent_entity_id):
        # Find Shrewsbury entity
        shrewsbury = next(e for e in seeded_entities if "Shrewsbury" in e["name"])
        
        response = api_client.get(f"{BASE_URL}/api/entities/{shrewsbury['id']}")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_nonexistent_entity_returns_404(self, api_client):
        ...
        response = api_client.get(f"{BASE_URL}/api/entities/{fake_id}")
...
class TestEntityCreateEndpoint:
    ...
    def test_create_entity_success(self, api_client, parent_entity_id):
        payload = {
        ...
        response = api_client.post(f"{BASE_URL}/api/entities", json=payload)
        ...
        # Cleanup
        api_client.delete(f"{BASE_URL}/api/entities/{data['id']}")
    
    def test_create_entity_with_valid_ch_number(self, api_client):
        payload = {
    ...
    def test_create_entity_with_sc_ch_number(self, api_client):
        payload = {
    ...
    def test_create_entity_invalid_ch_number_rejects_422(self, api_client):
        payload = {
    ...
    def test_create_entity_duplicate_ch_number_rejects_409(self, api_client):
        # Create first entity
        payload1 = {
        ...
        response1 = api_client.post(f"{BASE_URL}/api/entities", json=payload1)
        ...
        response2 = api_client.post(f"{BASE_URL}/api/entities", json=payload2)
        assert response2.status_code == 409
        assert "companies_house_number" in response2.json()["detail"]
        
        # Cleanup
        api_client.delete(f"{BASE_URL}/api/entities/{entity1_id}")
    
    def test_create_entity_duplicate_vat_number_rejects_409(self, api_client):
        # Create first entity
        payload1 = {
    ...
    def test_create_entity_bank_account_masked(self, api_client):
        payload = {
...
class TestEntityUpdateEndpoint:
    ...
    def test_update_entity_success(self, api_client):
        # Create entity
        payload = {
        ...
        create_response = api_client.post(f"{BASE_URL}/api/entities", json=payload)
        ...
        update_response = api_client.put(f"{BASE_URL}/api/entities/{entity_id}", json=update_payload)
        ...
        # Cleanup
        api_client.delete(f"{BASE_URL}/api/entities/{entity_id}")
    
    def test_update_entity_self_parent_rejects_400(self, api_client):
        # Create entity
        payload = {
    ...
    def test_update_entity_circular_parent_rejects_400(self, api_client):
        # Create entity A
        payload_a = {
        ...
        response_a = api_client.post(f"{BASE_URL}/api/entities", json=payload_a)
        ...
        response_b = api_client.post(f"{BASE_URL}/api/entities", json=payload_b)
        ...
        # Cleanup
        api_client.delete(f"{BASE_URL}/api/entities/{entity_b_id}")
        api_client.delete(f"{BASE_URL}/api/entities/{entity_a_id}")
    
    def test_update_entity_invalid_year_end_rejects_422(self, api_client):
        # Create entity
        payload = {
...
class TestEntityDeleteEndpoint:
    ...
    def test_delete_entity_with_children_rejects_409(self, api_client, parent_entity_id):
        # SY Homes Ltd has children, should not be deletable
        response = api_client.delete(f"{BASE_URL}/api/entities/{parent_entity_id}")
        assert response.status_code == 409
        assert "child" in response.json()["detail"].lower()
    
    def test_delete_leaf_entity_success(self, api_client):
        ...
        # Delete it
        delete_response = api_client.delete(f"{BASE_URL}/api/entities/{entity_id}")
        assert delete_response.status_code == 204
        
        # Verify it's gone
        get_response = api_client.get(f"{BASE_URL}/api/entities/{entity_id}")
...
class TestStruckOffStatus:
    ...
    def test_struck_off_hidden_by_default(self, api_client):
        ...
        list_response = api_client.get(f"{BASE_URL}/api/entities")
        ...
        filter_response = api_client.get(f"{BASE_URL}/api/entities", params={"status": "Struck_off"})
...
class TestInsuranceAlerts:
    ...
    def test_insurance_alerts_critical_severity(self, api_client):
        ...
        alerts_response = api_client.get(f"{BASE_URL}/api/meta/insurance-alerts")
    ...
    def test_insurance_alerts_warning_severity(self, api_client):
    ...
    def test_insurance_alerts_expired_severity(self, api_client):
...
class TestCleanup:
    ...
    def test_cleanup_test_entities(self, api_client):
        # Get all entities
        response = api_client.get(f"{BASE_URL}/api/entities", params={"page_size": 200, "include_struck_off": True})
